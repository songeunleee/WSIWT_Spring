...
export default function Closet({ category, temperature }) {
  ...
  console.log(closetQuery);

  let closetList = closetQuery && closetQuery;

  if (temperature) {
    closetList =
      closetQuery &&
      closetQuery.filter((item) => item.type.includes(getType(temperature)));
  }
  if (category) {
    closetList =
      closetQuery && closetQuery.filter((item) => item.category === category);
  }
