...
export default function Closet({ category, temperature }) {
  ...
  return (
    <div className=" bg-color2 rounded-3xl p-2">
      ...
      <ul className="p-2 px-4 rounded-3xl border-2 border-dashed border-color1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 w-full gap-2">
        {user &&
          closetList.map((item) => <ClosetList key={item.name} item={item} />)}
      </ul>
    </div>
  );
}
