...
export const publishedAt = (publishedAt, text) => {
  ...
  const tm = t / 60;
  if (tm < 60) return Math.floor(tm) + "분 전" + text !== undefined && text;
  const th = tm / 60;
  if (th < 24) return Math.floor(th) + "시간 전" + text !== undefined && text;
  const td = th / 24;
  if (td < 8) return Math.floor(td) + "일 전" + text !== undefined && text;
  const tw = td / 7;
  if (tw < 5) return Math.floor(tw) + "주 전" + text !== undefined && text;
  const tmon = td / 30;
  if (tw < 30) return Math.floor(tmon) + "달 전" + text !== undefined && text;
  const ty = td / 365;
  return Math.floor(ty) + "년 전" + text !== undefined && text;
};
