...
export default function NewClothes() {
  ...
  const [category, setCategory] = useState({ main: "", middle: "", sub: "" });
  ...
  const ClickMain = (value) => {
    setCategory({ main: value });
  };
  const ClickMiddle = (value, display) => {
    setCategory((pre) => ({ ...pre, middle: value, sub: "" }));
    setDisplay(display);
  };
  const ClickSub = (value, display) => {
    setCategory((pre) => ({ ...pre, sub: value }));
    setDisplay((prev) => (display === "기본" ? prev : display));
  };

  const handleSubit = (e) => {
    e.preventDefault();
    if (category.main === undefined || category.middle === undefined) {
        ...
        <div className="flex flex-col  bg-color2 rounded-md p-2 my-3">
          ...
          <div className="flex gap-1">
            ...
            <div className="flex flex-1 flex-col border-2 border-dashed border-color4 rounded-md">
              {category.main &&
                categoryList &&
                categoryList[category.main].map((item) => (
                  <CategoryItem
                    key={item.name}
                    item={item}
            ...
            <div className="flex flex-1 flex-col border-2 border-dashed border-color4 rounded-md">
              {category.middle &&
                categoryList &&
                categoryList[category.middle] &&
                categoryList[category.middle].map((item) => (
                  <CategoryItem
                    key={item.name}
                    item={item}
