...
export default function Main() {
  ...
  return (
    <section className="flex flex-col items-center h-full font-bold  ">
      <RegionInfo data={regionQuery.data} />
      <div className="  h-[40rem] mt-3 rounded-md overflow-y-scroll scrollbar-thin scrollbar-thumb-color4 scrollbar-track-color3  ">
        {weatherQuery.isLoading ? (
          <div>looading</div>
        ) : (
          <Swiper
            className="flex items-center max-w-xs sm:max-w-sm  xs:max-w-xs md:max-w-md  bg-color2   "
            initialSlide={0}
            autoHeight={true}
            pagination={{
              clickable: true,
