...
public class CommentApiController {
    ...
    @PostMapping("/api/v1/{ootdId}/comment")
    public ResponseEntity<String> save(@AuthenticationPrincipal String userId,@PathVariable Long ootdId, @RequestBody CommentSaveRequestDto requestDto){
            OOTD ootd = ootdService.findById(ootdId);
        Comment comment = Comment.builder().userId(userId).author(requestDto.getAuthor()).content(requestDto.getContent()).ootd(ootd).build();
